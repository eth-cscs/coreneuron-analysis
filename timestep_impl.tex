...
\begin{note}
    Many of the floating point operations in the \lst{state/rate/current} functions derived from the \hoc files are redundant. For example, in the example in \lstref{lst:state} no attempt is made to take advantage of identities like $\text{e}^{-x}=1/\text{e}^{x}$. Removing these operations during code generation would reduce the computational compexity of the code.
\end{note}
