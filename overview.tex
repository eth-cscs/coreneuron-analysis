...
There are also many instances where the naming of variables, global variables, and gratuitous use of the preprocessor don't help. It is recommeneded that the code is reformatted using a tool like \file{clang-format} (espcially the automatically generated C files in \file{mech/cfiles}). Applying the preprocessor to individual files shows the origin of symbols that have been \#defined with the preprocessor (e.g. using the \file{-S} flag in \lst{gcc})
